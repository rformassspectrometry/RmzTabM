test_that("smeCreate works", {
    ...
    expect_error(smeCreate(), "argument 'evidence_input_id' is mandatory")
    expect_error(smeCreate(evidence_input_id = evidence_input_id),
    ...
    res <- smeCreate(evidence_input_id = evidence_input_id,
                     exp_mass_to_charge = exp_mass_to_charge,
                     charge = charge,
                     theoretical_mass_to_charge = theoretical_mass_to_charge,
                     spectra_ref = spectra_ref,
    ...
    expect_error(smeCreate(c("bad", "arg"),
                           evidence_input_id = evidence_input_id,
                           exp_mass_to_charge = exp_mass_to_charge,
                           charge = charge,
                           theoretical_mass_to_charge = theoretical_mass_to_charge,
...
test_that("smeIdConfidenceMeasure works", {
    id_confidence_measure_vec <- c(1, 1.4, 1.2)
    mtd <- cbind(c("id_confidence_measure[1]"),
                 c("[MS, MS:1002890, fragmentation score, ]"))

    res <- smeIdConfidenceMeasure(id_confidence_measure_vec, mtd, nr = 3)
    expect_true(is.matrix(res))
    expect_true("id_confidence_measure[1]" %in% colnames(res))
    expect_true(nrow(res) == 3)

    expect_error(smeIdConfidenceMeasure(id_confidence_measure_vec, mtd,
                                        nr = 2),
    ...
    mtd_error = cbind(c("id_confidence_measure[1]", "id_confidence_measure[2]"),
                      c("[MS, MS:1002890, fragmentation score, ]",
                        "[MS, MS:1002890, fragmentation score, ]"))
    expect_error(smeIdConfidenceMeasure(id_confidence_measure_vec, mtd_error,
                                        nr = 3),
...
test_that("smeSpectraRefValidator validate", {
    expect_error(smeSpectraRefValidator(character(), matrix()),
                 "Provide a vector of \"spectra_ref\"")

    spectra_ref = c("ms_run[1]:index=7646", "ms_run[2]:index=7640",
                    "ms_run[3]:index=7671|ms_run[3]:index=7725")

    expect_error(smeSpectraRefValidator(spectra_ref, NULL),
                 "Provide a MTD section")
    expect_error(smeSpectraRefValidator(spectra_ref, matrix()),
    ...
    mtd_error <- cbind(c("ms_run[1]-location", "ms_run[1]-format",
                       ...
                       c("1.mzML", "[MS, MS:1000584, mzML file, ]",
                         "[MS, MS:1000530, mzML unique identifier, ]",
                         "[MS, MS:1000130, positive scan, ]"))
    expect_error(smeSpectraRefValidator(spectra_ref, mtd_error),
    ...
    mtd <- cbind(c("ms_run[1]-location", "ms_run[1]-format",
                 ...
                 c("1.mzML", "[MS, MS:1000584, mzML file, ]",
                   ...
                   "[MS, MS:1000530, mzML unique identifier, ]",
                   "[MS, MS:1000130, positive scan, ]"))
    res <- smeSpectraRefValidator(spectra_ref, mtd)
