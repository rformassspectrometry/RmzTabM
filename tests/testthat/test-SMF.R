test_that("smfCreate works", {
    mat <- matrix(1:4, nrow = 2)
    expect_error(smfCreate(mat), "argument 'exp_mass_to_charge' is mandatory")
    res <- smfCreate(x = mat, exp_mass_to_charge = c(100.1, 100.2))
    ...
    expect_error(smfCreate(c("bad", "arg"), x = mat,
                            exp_mass_to_charge = c(100.1, 100.2)),
                 "must be named")
    expect_error(smfCreate(x = mat, charge = c(1, 2, 3),
                            exp_mass_to_charge = c(100.1, 100.2)),
                 "does not match row count")
    expect_error(smfCreate(x = mat, exp_mass_to_charge = c(100.1, 100.2),
                            my_col = c(1, 2, 3)),
                 "does not match row count")
})
